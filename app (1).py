...
def main():
    ...
    with tab_citations:
        ...
        if citations_long is None:
            ...
            # All URLs overall (normalized, UTM params ignored)
            url_series = None
            url_col_name = None
            url_cols = [c for c in citations_long.columns if "url" in c.lower()] if citations_long is not None else []
            if url_cols:
                url_col_name = url_cols[0]
                url_series = citations_long[url_col_name]
            elif df_citations_src is not None:
                src_url_cols = [c for c in df_citations_src.columns if "url" in c.lower()]
                if src_url_cols:
                    url_col_name = src_url_cols[0]
                    url_series = df_citations_src[url_col_name]

            if url_series is not None:
                st.markdown("### All URLs overall")

                urls_norm = url_series.map(backend.normalize_url_for_count)
                urls_norm = urls_norm[urls_norm.astype(str).str.strip().ne("")]
                urls_norm = urls_norm[urls_norm.astype(str).str.lower().ne("nan")]
                ...
                if q.strip():
                    shown = shown[shown["url"].astype(str).str.contains(q.strip(), case=False, na=False)]

                st.caption(
                    f"Using URL column: {url_col_name}. URLs are normalized for counting (UTM parameters ignored)."
                )

...
def main():
    ...
    # Apply forced brand-family rules directly to the data.
    if hasattr(backend, "apply_forced_brand_rules"):
        mentions_long["brand"] = backend.apply_forced_brand_rules(mentions_long["brand"])
    ...
    try:
        ...
        if sugg is not None and (hasattr(sugg, "empty") and (not sugg.empty)):
            ...
            if existing_merge_map:
                normalized_merge_map = {
                    _norm_merge_value(k): _norm_merge_value(v)
                    for k, v in existing_merge_map.items()
                    if str(k).strip() and str(v).strip()
                }

                # Remove suggestions that are already actively applied,
                # so they do not keep reappearing after "Apply selected merges".
            ...
            # Preserve checked rows from the previous suggestions table across reruns,
            # so "Select all" and manual tick marks do not get reset.
    ...
    with merge_box.expander("Brand merge suggestions", expanded=False):
        ...
        if sugg_df is None:
            ...
            with st.form("manual_brand_merge_form", clear_on_submit=True):
                ...
                add_manual_merge = st.form_submit_button("Add")
            ...
            if add_manual_merge:
                ...
                if not alias or not canonical:
                    ...
                    tmp = sugg_df.copy()
                    if "combine" not in tmp.columns:
                        tmp.insert(0, "combine", False)
                    if "reason" not in tmp.columns:
                        tmp["reason"] = ""
                    if "score" not in tmp.columns:
                        tmp["score"] = np.nan

                    mask = (
                        tmp["alias"].astype(str).str.strip().eq(alias)
                        & tmp["canonical"].astype(str).str.strip().eq(canonical)
                    )
                    if mask.any():
                        tmp.loc[mask, "combine"] = True
                        if "reason" in tmp.columns:
                            tmp.loc[mask, "reason"] = tmp.loc[mask, "reason"].replace("", "manual")
                    else:
                        tmp = pd.concat(
                            [
                                tmp,
            ...
            with col_a:
                if st.button("Apply selected merges", type="primary", key="apply_brand_merges_btn"):
                    ...
                    if hasattr(backend_mod, "apply_merge_mapping"):
                        mentions_long2["brand"] = backend_mod.apply_merge_mapping(mentions_long2["brand"], existing_merge_map)
                    st.session_state["mentions_long"] = mentions_long2
                    st.rerun()
            with col_b:
                if st.button("Clear all merges", key="clear_brand_merges_btn"):
                    st.session_state["brand_merge_mapping"] = {}
                    st.rerun()

    current_merge_map = st.session_state.get("brand_merge_mapping", {})
    with st.expander("Active merges", expanded=False):
        if not current_merge_map:
            st.caption("No active merges applied.")
        else:
            active_df = pd.DataFrame(
                [{"remove": False, "alias": k, "canonical": v} for k, v in sorted(current_merge_map.items())]
            )
            ...
            with col_r1:
                if st.button("Remove selected active merges", key="remove_selected_active_merges_btn"):
                    to_remove = set(
                        active_edited.loc[active_edited["remove"] == True, "alias"].astype(str).str.strip().tolist()
                    )
                    new_map = {k: v for k, v in current_merge_map.items() if str(k).strip() not in to_remove}
                    st.session_state["brand_merge_mapping"] = new_map
                    st.rerun()
            with col_r2:

...
def main():
    ...
    with merge_box.expander("Brand merge suggestions", expanded=False):
        ...
        if sugg_df is None:
            ...
            with col_m1:
                manual_alias = st.text_input(
                    "Brand name to merge from",
                    value=st.session_state.get("manual_merge_alias", ""),
                    key="manual_merge_alias",
                )
            with col_m2:
                manual_canonical = st.text_input(
                    "Brand name to merge into",
                    value=st.session_state.get("manual_merge_canonical", ""),
                    key="manual_merge_canonical",
                )

            if st.button("Add", key="add_manual_brand_merge_btn"):
                ...
                if not alias or not canonical:
                    ...
                    st.session_state["brand_merge_suggestions"] = tmp
                    st.session_state["manual_merge_alias"] = ""
                    st.session_state["manual_merge_canonical"] = ""
                    st.rerun()

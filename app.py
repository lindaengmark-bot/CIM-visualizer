...
def _brand_merge_mapping_from_editor(df: pd.DataFrame) -> dict:
    ...
    if df is None or df.empty or "combine" not in df.columns:
        return {}
...
def main():
    ...
    # Recompute suggestions only when user clicks Run calculations
    if run_now:
        brand_counts = mentions_long["brand"].value_counts()
        if not hasattr(backend, "suggest_brand_merges"):
            st.error("Brand merge engine is not available in backend.py. Please update backend.py to the latest version.")
            sugg = None
        else:
            sugg = backend.suggest_brand_merges(
    ...
    with st.sidebar.expander("Brand merge suggestions", expanded=False):
        ...
        if sugg_df is None:
            ...
            with col_a:
                if st.button("Apply selected merges", type="primary", key="apply_brand_merges_btn"):
                    ...
                    mentions_long2["brand"] = backend.apply_merge_mapping(mentions_long2["brand"], merge_map)

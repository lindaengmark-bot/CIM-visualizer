...
# All URLs overall (from a URL column if available), normalized (UTM removed)
url_series = None
...
if url_series is not None:
    ...
    url_counts_all = urls_norm.value_counts().reset_index()
    url_counts_all.columns = ["url", "count"]
    st.session_state["url_counts_all"] = url_counts_all

    q = st.text_input("Search URLs", value="", placeholder="Type to filter", key="url_search_citations")
    min_count = st.number_input("Minimum count", min_value=1, value=1, step=1, key="url_min_count_citations")
...
url_counts_all = st.session_state.get("url_counts_all")
if url_counts_all is None and ("citations_long" in st.session_state) and st.session_state.get("citations_long") is not None:
    _cl = st.session_state.get("citations_long")
    _url_cols = [c for c in _cl.columns if "url" in c.lower()]
    if _url_cols:
        _urls_norm = _cl[_url_cols[0]].dropna().astype(str).map(backend.normalize_url_for_count)
        _urls_norm = _urls_norm[_urls_norm.str.strip() != ""]
        url_counts_all = _urls_norm.value_counts().reset_index()
        url_counts_all.columns = ["url", "count"]

        ...
        if sugg is not None and (hasattr(sugg, "empty") and (not sugg.empty)):
            ...
            if existing_merge_map:
                sugg["combine"] = sugg.apply(
                    lambda r: existing_merge_map.get(str(r.get("alias", "")).strip()) == str(r.get("canonical", "")).strip(),
                    axis=1,
                )
            elif previous_sugg is not None and not getattr(previous_sugg, "empty", True) and "combine" in previous_sugg.columns:
                prev_pairs = {
                    (str(r.get("alias", "")).strip(), str(r.get("canonical", "")).strip()): bool(r.get("combine", False))
                    for _, r in previous_sugg.iterrows()
                }
                sugg["combine"] = sugg.apply(
                    lambda r: prev_pairs.get(
                        (str(r.get("alias", "")).strip(), str(r.get("canonical", "")).strip()),
                        False,
                    ),
                    axis=1,
                )

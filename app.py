...
def _find_first_url_column(df: Optional[pd.DataFrame]) -> Optional[str]:
    if df is None:
        return None
    for c in df.columns:
...
def _compute_url_counts(series: pd.Series) -> pd.DataFrame:
    s = series.dropna().astype(str).map(backend.normalize_url_for_count)
    s = s[s.str.strip() != ""]
    s = s[s.str.lower() != "nan"]
    counts = s.value_counts().rename_axis("url").reset_index(name="count")
    return counts


def _compute_mention_url_counts_by_platform(mentions_long: pd.DataFrame) -> Optional[pd.DataFrame]:
    url_col = _find_first_url_column(mentions_long)
    if url_col is None:
        return None
    if "platform" not in mentions_long.columns:
        return None
    tmp = mentions_long[["platform", url_col]].copy()
    tmp["platform"] = tmp["platform"].astype(str).fillna("")
    tmp["url"] = tmp[url_col].astype(str).map(backend.normalize_url_for_count)
    tmp = tmp[(tmp["platform"].str.strip() != "") & (tmp["url"].str.strip() != "")]
    if tmp.empty:
        return pd.DataFrame(columns=["platform", "url", "count"])
    out = tmp.groupby(["platform", "url"]).size().reset_index(name="count")
    out = out.sort_values(["count", "platform", "url"], ascending=[False, True, True]).reset_index(drop=True)
    return out


def _compute_citation_url_counts_all(citations_long: Optional[pd.DataFrame], df_citations_src: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
    url_col = _find_first_url_column(citations_long)
    if url_col is not None and citations_long is not None:
        return _compute_url_counts(citations_long[url_col])
    url_col2 = _find_first_url_column(df_citations_src)
    if url_col2 is not None and df_citations_src is not None:
        return _compute_url_counts(df_citations_src[url_col2])
    return None
...
def main():
    ...
    with tab_mentions:
        ...
        mention_url_counts_by_platform = _compute_mention_url_counts_by_platform(mentions_long)
        if mention_url_counts_by_platform is None:
            st.info("No URL column found in mentions data, or platform is missing.")
        else:
            st.session_state["mention_url_counts_by_platform"] = mention_url_counts_by_platform

            # Filters (display-only)
            platforms = sorted(mention_url_counts_by_platform["platform"].dropna().astype(str).unique().tolist())
            sel_platforms = st.multiselect("Platforms to show", options=platforms, default=platforms[:10] if len(platforms) > 10 else platforms, key="mention_url_platforms")

            q = st.text_input("Search URLs", value="", placeholder="Type to filter URLs", key="mention_url_search")
            min_count = st.number_input("Minimum count", min_value=1, value=1, step=1, key="mention_url_min_count")

            shown = mention_url_counts_by_platform.copy()
            if sel_platforms:
                shown = shown[shown["platform"].isin(sel_platforms)]
            shown = shown[shown["count"] >= int(min_count)]
            if q.strip():
    ...
    with tab_citations:
        ...
        if citations_long is None:
            ...
            # All citation URLs overall (UTM-stripped normalization)
            ...
            if citation_url_counts_all is None:
                ...
                q2 = st.text_input("Search URLs", value="", placeholder="Type to filter URLs", key="cit_url_search")
                min_count2 = st.number_input("Minimum count", min_value=1, value=1, step=1, key="cit_url_min_count")

                shown2 = citation_url_counts_all[citation_url_counts_all["count"] >= int(min_count2)]
                if q2.strip():
                    shown2 = shown2[shown2["url"].str.contains(q2.strip(), case=False, na=False)]

                st.dataframe(shown2, use_container_width=True, hide_index=True)

                st.download_button(
                    "Download citation URL counts (CSV)",
    ...
    with tab_export:
        ...
        # URL summaries for export (computed even if you did not open the tabs)

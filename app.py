...
def _compute_citation_url_counts_all(
    citations_long: Optional[pd.DataFrame],
    df_citations_src: Optional[pd.DataFrame],
) -> Optional[pd.DataFrame]:
    """Compute normalized URL counts for citations.

    Looks for the first column containing 'url' (case-insensitive) in citations_long,
    otherwise falls back to df_citations_src. Normalizes URLs via backend.normalize_url_for_count.
    Returns a DataFrame with columns: url, count (sorted desc), or None if no URL column found.
    """
    url_series = None

    if citations_long is not None:
        url_cols = [c for c in citations_long.columns if "url" in str(c).lower()]
        if url_cols:
            url_series = citations_long[url_cols[0]]

    if url_series is None and df_citations_src is not None:
        src_url_cols = [c for c in df_citations_src.columns if "url" in str(c).lower()]
        if src_url_cols:
    ...
    s = url_series.dropna().astype(str).map(backend.normalize_url_for_count)
    s = s[s.str.strip() != ""]
    if s.empty:
        return pd.DataFrame(columns=["url", "count"])

    out = s.value_counts().rename_axis("url").reset_index(name="count")
    return out
...
def main():
    ...
    with tab_citations:
        ...
        if citations_long is None:
            ...
            # All URLs overall (UTM ignored) + search filter + export
            url_counts_all = _compute_citation_url_counts_all(citations_long, df_citations_src)
            st.session_state["citation_url_counts_all"] = url_counts_all

            if url_counts_all is None:
                ...
                min_count = st.number_input("Minimum count", min_value=1, value=1, step=1, key="cit_url_min_count")

                shown = url_counts_all.copy()
    ...
    with tab_export:
        ...
        # URL counts for export (citations). Computed here if not already available.
        url_counts_all = st.session_state.get("citation_url_counts_all")
        if url_counts_all is None and citations_long is not None:
            url_counts_all = _compute_citation_url_counts_all(citations_long, df_citations_src)
            st.session_state["citation_url_counts_all"] = url_counts_all
...
xlsx_bytes = backend.build_enhanced_excel(
        ...
        st.write(
            [
                ...
                "URLs_All: all normalized citation URLs with counts (UTM removed)",
            ]
        )

...
def _plot_counts_bar(df_counts: pd.DataFrame, title: str):
    st.subheader(title)
    if df_counts.empty:
        st.info("No data to plot.")
        return
    plot_df = df_counts.reset_index().melt(id_vars=df_counts.index.names, var_name="item", value_name="count")
    plot_df = plot_df[plot_df["count"] > 0]
    if px is None:
        st.dataframe(plot_df, use_container_width=True)
        st.caption("Install plotly to enable interactive charts.")
        return
    # If there is more than 1 group col, concatenate for a label
    idx_cols = [c for c in df_counts.index.names if c is not None]
    if idx_cols:
        plot_df["_group"] = plot_df[idx_cols].astype(str).agg(" | ".join, axis=1)
        fig = px.bar(plot_df, x="_group", y="count", color="item", barmode="stack", title=title)
        fig.update_layout(xaxis_title="Group", yaxis_title="Count", legend_title="Item")
    else:
        fig = px.bar(plot_df, x="item", y="count", title=title)
    st.plotly_chart(fig, use_container_width=True)


def _plot_share_heatmap(df_share: pd.DataFrame, title: str):
    st.subheader(title)
    if df_share.empty:
        st.info("No data to plot.")
        return
    ...
    if isinstance(mat.index, pd.MultiIndex):
        mat.index = mat.index.map(lambda t: " | ".join([str(x) for x in t]))
    fig = px.imshow(mat, aspect="auto", title=title)
    fig.update_layout(xaxis_title="Item", yaxis_title="Group")
    st.plotly_chart(fig, use_container_width=True)


def _maybe_date_filter(df: pd.DataFrame, date_col: str = "run_date") -> pd.DataFrame:
...
def main():
    ...
    with st.sidebar:
        ...
        selected_sheets = st.multiselect("Sheets to load", sheet_names, default=default_sheets or sheet_names[:1])
    ...
    st.sidebar.header("Normalization and mapping")
    mapping_lines = st.sidebar.text_area(
    ...
    st.sidebar.header("Mentions settings")
    # Choose brand columns depending on sheet
    brand_col_options = [c for c in df_mentions_src.columns if c.lower() in {"brands", "brand"}] + [c for c in df_mentions_src.columns if "brand" in c.lower()]
    brand_col = st.sidebar.selectbox("Brand column", options=brand_col_options or list(df_mentions_src.columns), index=0)
    inferred_col = None
    if "inferred_brands" in df_mentions_src.columns:
        inferred_col = st.sidebar.checkbox("Include inferred_brands (advanced Results)", value=False)
    inferred_brand_col = "inferred_brands" if inferred_col else None

    group_candidates = [c for c in ["platform", "model", "country", "run_date"] if c in df_mentions_src.columns]
    group_cols = st.sidebar.multiselect("Group by", options=group_candidates, default=[c for c in ["platform"] if c in group_candidates])
    top_n = st.sidebar.slider("Top N items", 5, 50, 25, 1)
    include_other = st.sidebar.checkbox("Include 'Other' bucket", value=True)
    ...
    mentions_long = _maybe_date_filter(mentions_long, "run_date")

    # Build citations long if present
    citations_long = None
    if df_citations_src is not None:
        # allow domain column choice
        dom_col_opts = [c for c in df_citations_src.columns if c.lower() in {"domain", "url"}] + [c for c in df_citations_src.columns if "domain" in c.lower()]
        domain_col = st.sidebar.selectbox("Citations domain column", options=dom_col_opts or list(df_citations_src.columns), index=0)
        citations_long = backend.build_citations_long(df_citations_src, domain_col=domain_col)
        citations_long = _maybe_date_filter(citations_long, "run_date")
    ...
    with tab_mentions:
        ...
        _plot_counts_bar(counts, "Mentions count (stacked by brand)")
        _plot_share_heatmap(share, "Mentions share (row-wise)")

        st.markdown("### Tables")
        st.write("Counts")
        st.dataframe(counts.reset_index(), use_container_width=True)
        st.write("Share")
        st.dataframe(share.reset_index(), use_container_width=True)
    ...
    with tab_sentiment:
        ...
        if "sentiment" not in sentiment_df.columns:
            ...
            sentiment_df2 = _maybe_date_filter(sentiment_df2, "run_date")
    ...
    with tab_citations:
        ...
        if citations_long is None:
            ...
            _plot_counts_bar(counts_c, "Citations count (stacked by domain)")
            _plot_share_heatmap(share_c, "Citations share (row-wise)")

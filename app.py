...
def main():
    ...
    # URL count tables (computed in tabs or during export)
    mention_url_counts_by_platform = None
    citation_url_counts_all = None

    def _compute_mention_url_counts_by_platform(mentions_long_df, mentions_src_df):
        if mentions_long_df is None or 'platform' not in mentions_long_df.columns:
            return None
        url_series = None
        url_cols = [c for c in mentions_long_df.columns if 'url' in str(c).lower()]
        if url_cols:
            url_series = mentions_long_df[url_cols[0]]
        elif mentions_src_df is not None:
            src_url_cols = [c for c in mentions_src_df.columns if 'url' in str(c).lower()]
            if src_url_cols:
                url_series = mentions_src_df[src_url_cols[0]]
        if url_series is None:
            return None
        tmp = pd.DataFrame({
            'platform': mentions_long_df['platform'].astype(str).replace('', 'Unknown').fillna('Unknown'),
            'url': url_series.astype(str).map(backend.normalize_url_for_count),
        })
        tmp = tmp[tmp['url'].str.strip() != '']
        if tmp.empty:
            return None
        out = (
            tmp.groupby(['platform', 'url']).size().reset_index(name='count')
            .sort_values(['count', 'platform', 'url'], ascending=[False, True, True])
        )
        return out

    def _compute_citation_url_counts_all(citations_long_df, citations_src_df):
        url_series = None
        if citations_long_df is not None:
            url_cols = [c for c in citations_long_df.columns if 'url' in str(c).lower()]
            if url_cols:
                url_series = citations_long_df[url_cols[0]]
        if url_series is None and citations_src_df is not None:
            src_url_cols = [c for c in citations_src_df.columns if 'url' in str(c).lower()]
            if src_url_cols:
                url_series = citations_src_df[src_url_cols[0]]
        if url_series is None:
            return None
        ...
        if urls_norm.empty:
            return None
        out = urls_norm.value_counts().rename_axis('url').reset_index(name='count')
        return out
    ...
    with tab_mentions:
        ...
        url_series_m = None
        url_cols_m = [c for c in mentions_long.columns if "url" in c.lower()] if mentions_long is not None else []
        if url_cols_m:
            url_series_m = mentions_long[url_cols_m[0]]
        elif df_mentions_src is not None:
            src_url_cols_m = [c for c in df_mentions_src.columns if "url" in c.lower()]
            if src_url_cols_m:
                url_series_m = df_mentions_src[src_url_cols_m[0]]

        if url_series_m is None:
            st.info("No URL column found in mentions data.")
        elif "platform" not in mentions_long.columns:
            st.info("No platform column found in mentions data, cannot show URLs by platform.")
        else:
            tmp_u = pd.DataFrame(
                {
                    "platform": mentions_long["platform"].astype(str),
                    "url": url_series_m.astype(str).map(backend.normalize_url_for_count),
                }
            )
            tmp_u["platform"] = tmp_u["platform"].replace("", "Unknown").fillna("Unknown")
            tmp_u = tmp_u[tmp_u["url"].str.strip() != ""]

            mention_url_counts_by_platform = (
                tmp_u.groupby(["platform", "url"])
                .size()
                .reset_index(name="count")
                .sort_values(["count", "platform", "url"], ascending=[False, True, True])
            )

            st.session_state["mention_url_counts_by_platform"] = mention_url_counts_by_platform

            platforms = sorted(mention_url_counts_by_platform["platform"].unique().tolist())
            sel_platforms = st.multiselect(
                "Platforms",
                options=platforms,
                default=platforms,
                key="mention_url_platforms",
            )
            q_m = st.text_input(
                "Search URLs",
                value="",
                placeholder="Type to filter",
                key="mention_url_search",
            )
            min_count_m = st.number_input(
                "Minimum count",
                min_value=1,
                value=1,
                step=1,
                key="mention_url_min_count",
            )

            shown_m = mention_url_counts_by_platform[
                (mention_url_counts_by_platform["platform"].isin(sel_platforms))
                & (mention_url_counts_by_platform["count"] >= int(min_count_m))
            ]
            if q_m.strip():
                shown_m = shown_m[shown_m["url"].str.contains(q_m.strip(), case=False, na=False)]

            st.dataframe(shown_m, use_container_width=True, hide_index=True)

            st.download_button(
                "Download mentioned URLs by platform (CSV)",
                data=mention_url_counts_by_platform.to_csv(index=False).encode("utf-8"),
                file_name="mention_urls_by_platform.csv",
                mime="text/csv",
                use_container_width=True,
            )
    ...
    with tab_citations:
        ...
        if citations_long is None:
            ...
            # All URLs overall (normalized, ignore UTM parameters)
            url_series = None
            ...
            if url_series is not None:
                ...
                urls_norm = url_series.dropna().astype(str).map(backend.normalize_url_for_count)
                urls_norm = urls_norm[urls_norm.str.strip() != ""]
                citation_url_counts_all = (
                    urls_norm.value_counts()
                    .rename_axis("url")
                    .reset_index(name="count")
                )

                st.session_state["citation_url_counts_all"] = citation_url_counts_all

                q = st.text_input(
                    "Search URLs",
                    value="",
                    placeholder="Type to filter, for example bygghemma",
                    key="citation_url_search",
                )
                min_count = st.number_input(
                    ...
                    value=1,
                    step=1,
                    key="citation_url_min_count",
                )

                shown = citation_url_counts_all[citation_url_counts_all["count"] >= int(min_count)]
                if q.strip():
                ...
                st.download_button(
                    ...
                    data=citation_url_counts_all.to_csv(index=False).encode("utf-8"),
                    file_name="citation_urls_all.csv",
                    mime="text/csv",
                    use_container_width=True,
                )
    ...
    with tab_export:
        ...
        # Ensure URL count tables exist for export (even if user did not open the tabs)
        mention_url_counts_by_platform = st.session_state.get("mention_url_counts_by_platform")
        if mention_url_counts_by_platform is None:
            mention_url_counts_by_platform = _compute_mention_url_counts_by_platform(mentions_long, df_mentions_src)

        citation_url_counts_all = st.session_state.get("citation_url_counts_all")
        if citation_url_counts_all is None:
            citation_url_counts_all = _compute_citation_url_counts_all(citations_long, df_citations_src)
        ...
        xlsx_bytes = backend.build_enhanced_excel(
            ...
            mention_url_counts_by_platform=mention_url_counts_by_platform,
            citation_url_counts_all=citation_url_counts_all,
        )
        st.download_button(

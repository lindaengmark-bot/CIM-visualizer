...
def main():
    ...
    with tab_citations:
        ...
        if citations_long is None:
            ...
            # Top URLs overall (from a URL column if available)
            url_series = None
            ...
            if url_series is not None:
                st.markdown("### Top URLs overall")
                top_urls = url_series.dropna().astype(str)
                top_urls = top_urls[top_urls.str.strip() != ""]
                top_urls = top_urls.value_counts().head(top_n_c).reset_index()

...
def main():
    ...
    with tab_export:
        ...
        # Ensure URL counts are available for export, even if the Citations tab was not opened
        if citations_long is not None and st.session_state.get("url_counts_all") is None:
            url_cols_export = [c for c in citations_long.columns if "url" in str(c).lower()]
            if url_cols_export:
                urls_norm = citations_long[url_cols_export[0]].dropna().astype(str).map(backend.normalize_url_for_count)
                urls_norm = urls_norm[urls_norm.str.strip() != ""]
                st.session_state["url_counts_all"] = (
                    urls_norm.value_counts()
                    .rename_axis("url")
                    .reset_index(name="count")
                )

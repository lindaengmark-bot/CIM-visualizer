...
def main():
    ...
    # Apply active exact merges to the freshly built dataset.
    active_exact_merges = st.session_state.get("brand_merge_mapping", {})
    if active_exact_merges and hasattr(backend_mod, "apply_merge_mapping"):
        mentions_long["brand"] = backend_mod.apply_merge_mapping(
            mentions_long["brand"],
            active_exact_merges,
        )

    # Final safety pass: same brand with different casing should always be merged.
    if hasattr(backend_mod, "auto_merge_exact_case"):
        mentions_long["brand"] = backend_mod.auto_merge_exact_case(mentions_long["brand"])
    ...
    try:
        ...
        if sugg is not None and (hasattr(sugg, "empty") and (not sugg.empty)):
            ...
            # Remove suggestions that are already active.
            active_exact_merges = st.session_state.get("brand_merge_mapping", {})
            ...
            # Preserve checked rows across reruns, so Select all and manual ticks are not reset.
            ...
            if previous_sugg is not None and hasattr(previous_sugg, "empty") and not previous_sugg.empty:
                if "reason" in previous_sugg.columns:
                    manual_rows = previous_sugg[previous_sugg["reason"].astype(str).str.lower() == "manual"].copy()

            if not manual_rows.empty:
                for col in ["combine", "alias", "canonical", "reason", "score"]:
                    if col not in manual_rows.columns:
                        manual_rows[col] = np.nan
                manual_rows = manual_rows[["combine", "alias", "canonical", "reason", "score"]]
                if sugg is None or getattr(sugg, "empty", True):
                    sugg = manual_rows.copy()
                else:
                    if "combine" not in sugg.columns:
                        sugg.insert(0, "combine", False)
                    if "reason" not in sugg.columns:
                        sugg["reason"] = ""
                    if "score" not in sugg.columns:
                        sugg["score"] = np.nan
                    sugg = pd.concat([sugg, manual_rows], ignore_index=True)

                sugg["alias_norm"] = sugg["alias"].astype(str).str.strip().str.lower()
                sugg["canonical_norm"] = sugg["canonical"].astype(str).str.strip().str.lower()
                sugg = sugg.drop_duplicates(
                    subset=["alias_norm", "canonical_norm"],
                    keep="last",
                ).drop(columns=["alias_norm", "canonical_norm"])

            if sugg is not None and not getattr(sugg, "empty", True) and "combine" not in sugg.columns:
                sugg.insert(0, "combine", False)

            if sugg is not None and not getattr(sugg, "empty", True):
                sugg["combine"] = sugg.apply(
                    lambda r: bool(r.get("combine", False)) or (
    ...
    with merge_box.expander("Suggested brand merges", expanded=False):
        ...
        if sugg_df is None:
            ...
            st.write("Select pairs to combine. You can edit the Canonical value before applying.")
            ...
            edited = st.data_editor(
                ...
                column_config={
                    ...
                    "canonical": st.column_config.TextColumn("Canonical"),
                },
            )
    ...
    with merge_box.expander("Add manual brand merges", expanded=False):
        st.write("Add one or several manual merges. They will be added to the Suggested brand merges list for review before applying.")
        st.caption("Use one row per merge, for example: black decker = Black & Decker")

        with st.form("manual_brand_merge_form", clear_on_submit=True):
            manual_merge_lines = st.text_area(
                "Manual merges",
                value="",
                placeholder="black decker = Black & Decker\nblack-decker = Black & Decker",
                height=120,
                key="manual_merge_lines_input",
            )
            add_manual_merges = st.form_submit_button("Add to suggested merges")

        if add_manual_merges:
            rows_to_add = []
            errors = []

            for line_no, raw_line in enumerate(str(manual_merge_lines).splitlines(), start=1):
                line = raw_line.strip()
                if not line:
                    continue
                ...
                if "=" in line:
                    ...
                    errors.append(f"Line {line_no}: use '=' or '->' between alias and canonical.")
                    continue
                ...
                if not alias or not canonical:
                    errors.append(f"Line {line_no}: both alias and canonical are required.")
                    continue
                if alias.lower() == canonical.lower():
                    errors.append(f"Line {line_no}: alias and canonical must be different.")
                    continue
            ...
            if rows_to_add:
                existing_sugg = st.session_state.get("brand_merge_suggestions")
                if existing_sugg is None or getattr(existing_sugg, "empty", True):
                    updated_sugg = pd.DataFrame(rows_to_add)
                else:
                    updated_sugg = existing_sugg.copy()
                    if "combine" not in updated_sugg.columns:
                        updated_sugg.insert(0, "combine", False)
                    if "reason" not in updated_sugg.columns:
                        updated_sugg["reason"] = ""
                    if "score" not in updated_sugg.columns:
                        updated_sugg["score"] = np.nan

                    updated_sugg = pd.concat([updated_sugg, pd.DataFrame(rows_to_add)], ignore_index=True)

                # Keep the latest manual value for duplicate alias/canonical pairs
                updated_sugg["alias_norm"] = updated_sugg["alias"].astype(str).str.strip().str.lower()
                updated_sugg["canonical_norm"] = updated_sugg["canonical"].astype(str).str.strip().str.lower()
                updated_sugg = updated_sugg.drop_duplicates(
                    subset=["alias_norm", "canonical_norm"],
                    keep="last",
                ).drop(columns=["alias_norm", "canonical_norm"])

                st.session_state["brand_merge_suggestions"] = updated_sugg.reset_index(drop=True)
                st.success(f"Added {len(rows_to_add)} manual merge(s) to Suggested brand merges.")
                st.rerun()

    with merge_box.expander("Active merges", expanded=False):
        active_exact_merges = st.session_state.get("brand_merge_mapping", {})
        if not active_exact_merges:
            st.caption("No active merges applied.")
        else:
            active_df = pd.DataFrame(
                [{"remove": False, "alias": k, "canonical": v} for k, v in sorted(active_exact_merges.items())]
            )
            active_edited = st.data_editor(
                ...
                column_config={
                    ...
                    "alias": st.column_config.TextColumn("Alias", disabled=True),
                    "canonical": st.column_config.TextColumn("Canonical", disabled=True),
                },
            )

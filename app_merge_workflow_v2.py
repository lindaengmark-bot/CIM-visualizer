...
def main():
    ...
    with merge_box.expander("Rule-based brand merges", expanded=False):
        if hasattr(backend_mod, "get_forced_brand_rule_hits"):
            rule_counts = mentions_long["brand"].value_counts()
        ...
        if rule_hits is None or getattr(rule_hits, "empty", True):
            st.caption("No rule-based merge matches found for the current data.")
        else:
            active_rule_ids = {str(r.get("rule_id", "")) for r in st.session_state.get("active_rule_merges", [])}
            rule_hits = rule_hits.copy()
            rule_hits["apply"] = ~rule_hits["rule_id"].astype(str).isin(active_rule_ids)

            st.write("Review known product-family rules before using fuzzy suggestions.")
            rule_display = st.data_editor(
                rule_hits[["apply", "match", "canonical", "matched_brands", "matched_count"]],
                use_container_width=True,
                hide_index=True,
                key="rule_based_merge_editor",
                column_config={
                    "apply": st.column_config.CheckboxColumn("Apply"),
                    "match": st.column_config.TextColumn("Rule match", disabled=True),
                    "canonical": st.column_config.TextColumn("Merge into", disabled=True),
                    "matched_brands": st.column_config.TextColumn("Matched brands", disabled=True),
                    "matched_count": st.column_config.NumberColumn("Mentions", disabled=True),
                },
            )

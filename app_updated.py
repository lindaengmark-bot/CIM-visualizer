...
def main():
    ...
    if run_now:
        ...
        suggestions = backend.suggest_brand_merges(
        ...
        if not suggestions.empty:
            suggestions.insert(0, "combine", False)

        st.session_state["brand_merge_suggestions"] = suggestions
    ...
    with st.sidebar.expander("Brand merge suggestions", expanded=False):
        ...
        if sugg_df is None:
            ...
            st.caption("No merge suggestions found.")
        else:
            ...
            if st.button("Apply selected merges", type="primary"):
                merge_map = {
                    ...
                    if row["combine"] is True
                }
                mentions_long["brand"] = backend.apply_merge_mapping(
                    mentions_long["brand"],
                    merge_map
                )
                st.session_state["mentions_long"] = mentions_long

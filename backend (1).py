...
def normalize_url_for_count(url: object) -> str:
    """Normalize URLs for counting mentions.

    Rules:
    - Ignore UTM query params (keys starting with 'utm_', case-insensitive)
    - Drop fragments
    - Normalize scheme to https
    - Lowercase hostname and remove leading 'www.'
    - Remove trailing slash for non-root paths
    - Preserve non-UTM query parameters (sorted) so meaningful query differences remain
    """
    s = _normalize_text(url)
    if not s:
        return ""

    # URLs without scheme become paths in urlsplit, assume https.
...
def build_enhanced_excel(
    ...
    url_counts_all: Optional[pd.DataFrame] = None,
) -> bytes:
    """
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if url_counts_all is not None and not url_counts_all.empty:
            url_counts_all.to_excel(writer, sheet_name="URLs_All", index=False)

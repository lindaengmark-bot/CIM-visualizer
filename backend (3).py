    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        def _write_summary(df: Optional[pd.DataFrame], sheet: str):
            ...
            if isinstance(out.index, pd.MultiIndex):
                index_names = [str(n).strip().lower() for n in out.index.names if n is not None]
                out = out.reset_index()
            elif out.index.name is not None:
                index_names = [str(out.index.name).strip().lower()]
                out = out.reset_index()

            platform_cols = [c for c in out.columns if str(c).strip().lower() == "platform"]
            if platform_cols:
                platform_col = platform_cols[0]
                other_cols = [c for c in out.columns if c != platform_col]
                out = out[[platform_col] + other_cols]
                out = out.rename(columns={platform_col: "Platform"})
            elif "platform" in index_names and len(out.columns) > 0:
                out = out.rename(columns={out.columns[0]: "Platform"})

...
def build_enhanced_excel(
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        def _prepare_summary_for_export(df: pd.DataFrame, sheet: str) -> pd.DataFrame:
            out = df.copy()
            target_sheets = {"Mentions_Counts", "Mentions_Share", "Citations_Counts", "Citations_Share"}
            if sheet in target_sheets:
                if isinstance(out.index, pd.MultiIndex):
                    names = ["Platform" if str(name).lower() == "platform" else name for name in out.index.names]
                    out.index = out.index.set_names(names)
                elif str(out.index.name).lower() == "platform":
                    out.index.name = "Platform"
                if "platform" in out.columns:
                    out = out.rename(columns={"platform": "Platform"})
            return out

        def _write_summary(df: Optional[pd.DataFrame], sheet: str):
            if df is None:
                return
            out = _prepare_summary_for_export(df, sheet)
            if isinstance(out.index, pd.MultiIndex):
                out.reset_index().to_excel(writer, sheet_name=sheet[:31], index=False)
            elif out.index.name is not None:
                out.to_excel(writer, sheet_name=sheet[:31], index=True)
            else:

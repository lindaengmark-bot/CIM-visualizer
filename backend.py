...
def normalize_url_for_count(url: object) -> str:
    """Normalize URLs for counting citations.
    ...
    - Preserve non-UTM query parameters (sorted)
    """
    ...
    keep_params.sort(key=lambda kv: (kv[0], kv[1]))
...
def build_enhanced_excel(
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        # All normalized citation URLs with counts (optional)
        if url_counts_all is not None:
            try:

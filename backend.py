...
def normalize_url_for_count(url: object) -> str:
    ...
    - Removes UTM parameters (any query key starting with 'utm_' case-insensitive)
    - Removes fragments (#...)
    - Normalizes scheme to https
    - Lowercases hostname and removes leading 'www.'
    - Removes trailing slash (except root)
    - Keeps non-UTM query params (sorted) so meaningful query differences remain distinct
    """
    s = _normalize_text(url).strip()
    if not s:
        return ""
    try:
        parsed = urlparse(s)
        scheme = parsed.scheme.lower() if parsed.scheme else "https"
        scheme = "https"  # collapse http/https for counting
        ...
        rebuilt = urlunparse((scheme, host, path, "", new_query, ""))  # drop fragment
        return rebuilt
    except Exception:
        # Fallback: strip utm params crudely
        s2 = re.sub(r"[#].*$", "", s)
        s2 = re.sub(r"([?&])utm_[^=&]+=[^&]*", r"\1", s2, flags=re.IGNORECASE)
        s2 = re.sub(r"[?&]+$", "", s2)
        return s2.strip()

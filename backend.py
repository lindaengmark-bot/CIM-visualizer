...
def normalize_url_for_count(url: object) -> str:
    ...
    - remove utm_* query params (case-insensitive)
    - drop fragments
    - normalize scheme to https
    - lowercase hostname and strip leading www.
    - remove trailing slash for non-root paths
    - keep non-UTM query params (sorted) so meaningful query differences remain distinct
    """
    ...
    # remove utm_* params, keep others
    qs = []
    try:
...
def build_url_counts(
    df: pd.DataFrame,
    url_col: str,
    group_col: Optional[str] = None,
) -> pd.DataFrame:
    """
    Build a URL counts table from df[url_col], normalized with normalize_url_for_count.
    If group_col is provided and present, returns counts per group.
    Output columns:
      - if group_col: [group_col, "url", "count"]
      - else: ["url", "count"]
    """
    if df is None or df.empty or url_col not in df.columns:
        return pd.DataFrame(columns=([group_col, "url", "count"] if group_col else ["url", "count"]))

    s = df[url_col].dropna().astype(str).map(normalize_url_for_count)
    s = s[s.str.strip() != ""]
    if s.empty:
        return pd.DataFrame(columns=([group_col, "url", "count"] if group_col else ["url", "count"]))

    if group_col and group_col in df.columns:
        tmp = pd.DataFrame({group_col: df.loc[s.index, group_col].astype(str), "url": s})
        out = (
            tmp.groupby([group_col, "url"], dropna=False)
            .size()
            .reset_index(name="count")
            .sort_values(["count", group_col, "url"], ascending=[False, True, True])
...
def build_enhanced_excel(
    ...
    mention_url_counts: Optional[pd.DataFrame] = None,
    citation_url_counts: Optional[pd.DataFrame] = None,
) -> bytes:
    """
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if mention_url_counts is not None and not mention_url_counts.empty:
            mention_url_counts.to_excel(writer, sheet_name="Mention_URLs_ByPlatform"[:31], index=False)
        if citation_url_counts is not None and not citation_url_counts.empty:
            citation_url_counts.to_excel(writer, sheet_name="Citation_URLs_All"[:31], index=False)

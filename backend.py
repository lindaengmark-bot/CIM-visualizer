...
# ============================================================
# Brand Similarity Engine (suggest merges + user-approved apply)
# ============================================================

from difflib import SequenceMatcher

def _normalize_for_similarity(s: str) -> str:
    """Normalization for fuzzy brand matching.
    Rules:
      - case-insensitive
      - ä->a, ö->o, ü->u
...
def suggest_brand_merges(
    ...
    """Return a DataFrame of suggested merges with columns:
    alias, canonical, reason, score.

    Criteria:
      1) Exact match (case-insensitive)
      2) Name contained within another (min length >= 3)
      3) >=80% character similarity after special normalization
    """
    ...
    norm = {b: _normalize_for_similarity(b) for b in uniq}

    def _pick_canonical(a: str, b: str) -> str:
        ca = float(counts.get(a, 0))
        cb = float(counts.get(b, 0))
        if ca > cb:
            return a
        if cb > ca:
            return b
        return a if len(norm.get(a, "")) >= len(norm.get(b, "")) else b
    ...
    for i in range(len(uniq)):
        for j in range(i + 1, len(uniq)):
            ...
            # 1) exact
            if normalize_brand(a).lower() == normalize_brand(b).lower():
                canonical = _pick_canonical(a, b)
                alias = b if canonical == a else a
                key = (alias, canonical)
                if key not in seen:
                    seen.add(key)
                    suggestions.append((alias, canonical, "exact (case-insensitive)", 1.0))
                continue
            ...
            if len(short) >= min_containment_len and short in long_:
                ...
                if key not in seen:
                    seen.add(key)
                    suggestions.append((alias, canonical, "contained name", float(sim)))
                continue
            ...
            if sim >= similarity_threshold:
                ...
                if key not in seen:
                    seen.add(key)
                    suggestions.append((alias, canonical, ">=80% similarity", float(sim)))

    df = pd.DataFrame(suggestions, columns=["alias", "canonical", "reason", "score"])
    if df.empty:
        return df
    df = df.sort_values(["score", "reason"], ascending=[False, True]).head(max_suggestions).reset_index(drop=True)
    return df


def apply_merge_mapping(brands: pd.Series, mapping: Dict[str, str]) -> pd.Series:
    """Apply alias->canonical mapping, case-insensitive after normalize_brand."""
    if not mapping:
        return brands

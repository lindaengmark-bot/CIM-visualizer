...
def apply_forced_brand_rules(brands: pd.Series) -> pd.Series:
    """Apply hard-coded brand normalization rules directly to the data."""
    def _apply_one(value: object) -> str:
        b = normalize_brand(value)
        if not b:
            return b

        if _automower_like(b):
            return "Husqvarna"
        if _matches_forced_brand(b, "Landorid"):
            return "Worx"
        if _matches_forced_brand(b, "LUBA"):
            return "Mammotion"
        if _matches_forced_brand(b, "Yuka"):
            return "Mammotion"
        if _matches_forced_brand(b, "Navimow"):
            return "Segway"
        if _matches_forced_brand(b, "iMOW"):
            return "STIHL"
        if _matches_forced_brand(b, "EasiLife"):
            return "Flymo"
        if _matches_forced_brand(b, "UltraLife"):
            return "Flymo"
...
def suggest_brand_merges(
    ...
    forced_merge_rules = [
        (lambda b: _automower_like(b), "Husqvarna", "forced automower -> Husqvarna"),
        (lambda b: _matches_forced_brand(b, "Landorid"), "Worx", "forced landorid -> Worx"),
        (lambda b: _matches_forced_brand(b, "LUBA"), "Mammotion", "forced LUBA -> Mammotion"),
        (lambda b: _matches_forced_brand(b, "Yuka"), "Mammotion", "forced Yuka -> Mammotion"),
        (lambda b: _matches_forced_brand(b, "Navimow"), "Segway", "forced Navimow -> Segway"),
        (lambda b: _matches_forced_brand(b, "iMOW"), "STIHL", "forced iMOW -> STIHL"),
        (lambda b: _matches_forced_brand(b, "EasiLife"), "Flymo", "forced EasiLife -> Flymo"),
        (lambda b: _matches_forced_brand(b, "UltraLife"), "Flymo", "forced UltraLife -> Flymo"),
    ]

    for b in uniq:
        for matcher, canonical_target, reason in forced_merge_rules:
            if matcher(b) and normalize_brand(b).lower() != normalize_brand(canonical_target).lower():
                rows.append(
                    {
                        "alias": b,

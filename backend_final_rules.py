...
def normalize_url_for_count(url: object) -> str:
    """Normalize URLs for counting.

    Removes UTM parameters (keys starting with 'utm_' case-insensitive), fragments, normalizes scheme,
    lowercases hostname and strips leading 'www.', removes trailing slash for non-root paths.
    Keeps other query parameters (sorted) so meaningful query differences stay distinct.
    """
    if url is None:
        return ""
    s = str(url).strip()
...
def build_enhanced_excel(
    ...
    url_counts_all: pd.DataFrame | None = None,
) -> bytes:
    """
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        def _write_summary(df: Optional[pd.DataFrame], sheet: str):
            ...
            if isinstance(out.index, pd.MultiIndex):
                ...
                platform_cols = ["platform"]
                other_cols = [c for c in out.columns if c != "platform"]
                out = out[platform_cols + other_cols]
            elif getattr(out.index, "dtype", None) == object and len(out.index) == len(out) and len(out.index) > 0:
                platform_like = out.index.astype(str).str.strip().ne("").all()
                if platform_like:
                    out = out.reset_index()

            rename_map = {}
            if len(out.columns) > 0:
                first_col = str(out.columns[0]).strip().lower()
                if first_col in {"platform", "index", "level_0"}:
                    rename_map[out.columns[0]] = "Platform"
...
def _compact_brand_token(s: str) -> str:
    return re.sub(r"[^a-z0-9]+", "", normalize_brand(s).lower())


def _automower_like(s: str) -> bool:
    compact = _compact_brand_token(s)
    return compact == "automower"


def _normalize_for_forced_match(s: str) -> str:
    s2 = normalize_brand(s).lower()
    s2 = re.sub(r"[^a-z0-9]+", " ", s2)
    s2 = re.sub(r"\s+", " ", s2).strip()
    return s2
...
def _matches_forced_brand(s: str, token: str) -> bool:
    """Match a forced brand rule robustly, without accidental substring hits.

    Examples that should match:
    - 'Navimow', 'Navimow i105', 'NAVIMOW'
    - 'iMOW', 'iMOW 5', 'IMOW'
    - 'LUBA 2', 'Yuka Mini'

    But this should NOT happen:
    - 'Navimow' matching 'iMOW' just because 'imow' is a substring.
    """
    compact_s = _compact_brand_token(s)
    compact_token = _compact_brand_token(token)
    if not compact_s or not compact_token:
        return False

    # Exact or prefix match on compact form, so 'navimowi105' matches 'navimow'
    # but 'navimow' does not match 'imow'.
    if compact_s == compact_token or compact_s.startswith(compact_token):
        return True

    # Also support separated words or punctuation variants via whole-phrase matching.
    spaced_s = _normalize_for_forced_match(s)
    spaced_token = _normalize_for_forced_match(token)
    if not spaced_s or not spaced_token:
        return False

    pattern = r"\b" + re.escape(spaced_token).replace(r"\ ", r"\s+") + r"\b"
    return re.search(pattern, spaced_s) is not None
...
# ============================================================
# Forced brand rule engine
# ============================================================


FORCED_BRAND_RULES = [
    {"rule_id": "automower_to_husqvarna", "match": "Automower", "canonical": "Husqvarna"},

    {"rule_id": "landorid_to_worx", "match": "Landorid", "canonical": "Worx"},
    {"rule_id": "landroid_to_worx", "match": "Landroid", "canonical": "Worx"},

    {"rule_id": "luba_to_mammotion", "match": "LUBA", "canonical": "Mammotion"},
    {"rule_id": "yuka_to_mammotion", "match": "Yuka", "canonical": "Mammotion"},

    {"rule_id": "navimow_to_segway", "match": "Navimow", "canonical": "Segway"},

    {"rule_id": "sileno_to_gardena", "match": "Sileno", "canonical": "Gardena"},
    {"rule_id": "indego_to_bosch", "match": "Indego", "canonical": "Bosch"},
    {"rule_id": "visimow_to_bosch", "match": "Visimow", "canonical": "Bosch"},
    {"rule_id": "vista_to_stiga", "match": "Vista", "canonical": "Stiga"},

    {"rule_id": "imow_to_stihl", "match": "iMOW", "canonical": "STIHL"},
    {"rule_id": "easilife_to_flymo", "match": "EasiLife", "canonical": "Flymo"},
    {"rule_id": "ultralife_to_flymo", "match": "UltraLife", "canonical": "Flymo"},
]



def _forced_rule_matches_brand(brand: str, match: str) -> bool:
    b_compact = _compact_brand_token(brand)
    m_compact = _compact_brand_token(match)
    if not b_compact or not m_compact:
        return False
    if b_compact == m_compact or b_compact.startswith(m_compact):
        return True

    b_spaced = _normalize_for_forced_match(brand)
    m_spaced = _normalize_for_forced_match(match)
    if not b_spaced or not m_spaced:
        return False
    pattern = r"\b" + re.escape(m_spaced).replace(r"\ ", r"\s+") + r"\b"
    return re.search(pattern, b_spaced) is not None


def _brand_belongs_to_forced_rule(brand: str, rule: dict) -> bool:
    """Broad detection used only for showing rule-based candidates.

    This is intentionally broader than exact alias matching:
    if a brand contains the rule token anywhere, it should appear in Rule-based,
    not Suggested. Nothing is applied until the user actively selects it.
    """
    b = normalize_brand(brand)
    if not b:
        return False

    canonical = normalize_brand(rule.get("canonical", ""))
    if canonical and b.lower() == canonical.lower():
        return False

    match = normalize_brand(rule.get("match", ""))
    if not match:
        return False

    b_compact = _compact_brand_token(b)
    m_compact = _compact_brand_token(match)

    # Safety for common spelling variants around Landorid / Landroid.
    if rule.get("canonical") == "Worx" and ("landroid" in b_compact or "landorid" in b_compact):
        return True

    if m_compact and m_compact in b_compact:
        return True

    b_spaced = _normalize_for_forced_match(b)
    m_spaced = _normalize_for_forced_match(match)
    if m_spaced and m_spaced in b_spaced:
        return True

    return _forced_rule_matches_brand(b, match)


def get_forced_brand_rule_hits(brands, counts=None) -> pd.DataFrame:
    ...
    rows = []
    for rule in FORCED_BRAND_RULES:
        matched = []
        total_count = 0
        for b in uniq:
            if _brand_belongs_to_forced_rule(b, rule):
                matched.append(normalize_brand(b))
                total_count += int(counts.get(b, 0))
        if matched:
            rows.append({
                "apply": False,
                "type": "Rule",
                ...
                "matched_brands": ", ".join(sorted(set(matched))[:20]),
                "matched_count": total_count,
            })

    return pd.DataFrame(rows, columns=["apply", "type", "rule_id", "match", "canonical", "matched_brands", "matched_count"])


def apply_forced_rule_merges(brands: pd.Series, active_rules) -> pd.Series:
    if active_rules is None:
        return brands

    if isinstance(active_rules, pd.DataFrame):
        rules = active_rules.to_dict("records")
    else:
    ...
    for r in rules:
        if isinstance(r, dict):
            ...
            if match and canonical:
                selected.append({"match": match, "canonical": canonical})

    if not selected:
        return brands

    def _apply_one(value: object) -> str:
        b = normalize_brand(value)
        if not b:
            return b
        b_compact = _compact_brand_token(b)
        for rule in selected:
            canonical = normalize_brand(rule["canonical"])
            match = normalize_brand(rule["match"])
            if canonical == "Worx" and ("landroid" in b_compact or "landorid" in b_compact):
                return canonical
            if _forced_rule_matches_brand(b, match):
                return canonical
        return b

    return brands.fillna("").astype(str).map(_apply_one)


def apply_forced_brand_rules(brands: pd.Series) -> pd.Series:
    return apply_forced_rule_merges(brands, FORCED_BRAND_RULES)


def apply_exact_merge_mapping(brands: pd.Series, mapping: Dict[str, str]) -> pd.Series:
    return apply_merge_mapping(brands, mapping)


def is_forced_brand_rule_match(brand: str) -> bool:
    """True if a brand belongs to any forced rule family.

    Used to keep rule-based brands out of fuzzy suggestions.
    """
    return any(_brand_belongs_to_forced_rule(brand, rule) for rule in FORCED_BRAND_RULES)
...
def suggest_brand_merges(
    ...
    df["alias"] = df["alias"].astype(str).map(normalize_brand)
    df["canonical"] = df["canonical"].astype(str).map(normalize_brand)
    df = df[df["alias"] != ""]
    df = df[df["canonical"] != ""]
    df = df[df["alias"].str.lower() != df["canonical"].str.lower()]
    df = df.drop_duplicates(subset=["alias", "canonical"], keep="first")

...
def _automower_like(s: str) -> bool:
    compact = re.sub(r"\s+", "", normalize_brand(s).lower())
...
def suggest_brand_merges(
    ...
    # Forced rule: any Automower variant should always be suggested to merge into Husqvarna.
    for b in uniq:
        if _automower_like(b) and normalize_brand(b).lower() != "husqvarna":
            rows.append(
                {
                    "alias": b,
                    "canonical": "Husqvarna",
                    "reason": "forced automower -> Husqvarna",
                    "score": 1.0,
                }
            )

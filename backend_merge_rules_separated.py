...
def get_forced_brand_rule_hits(brands, counts=None) -> pd.DataFrame:
    ...
    for rule in FORCED_BRAND_RULES:
        ...
        total_count = 0
        for b in uniq:
            if _forced_rule_matches_brand(b, rule["match"]) and normalize_brand(b).lower() != normalize_brand(rule["canonical"]).lower():
...
def is_forced_brand_rule_match(brand: str) -> bool:
    ...
    """
    for rule in FORCED_BRAND_RULES:
        if _forced_rule_matches_brand(brand, rule["match"]) and normalize_brand(brand).lower() != normalize_brand(rule["canonical"]).lower():
            return True
    return False

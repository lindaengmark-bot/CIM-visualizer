...
# ============================================================
# Brand Similarity Engine (Auto Merge Suggestions)
# ============================================================
...
def suggest_brand_merges(brands, counts=None, similarity_threshold=0.80):
    import pandas as pd
    ...
    for i in range(len(uniq)):
        for j in range(i + 1, len(uniq)):
            ...
            if normalize_brand(a).lower() == normalize_brand(b).lower():
                suggestions.append((a, b, "exact", 1.0))
                continue
            ...
            if len(short) >= 3 and short in long_:
                ...
                suggestions.append((a, b, "contained", sim))
                continue

            # Similarity
            sim = _char_similarity(a, b)
            if sim >= similarity_threshold:
                suggestions.append((a, b, "similarity", sim))

    df = pd.DataFrame(suggestions, columns=["alias", "canonical", "reason", "score"])
    return df.sort_values("score", ascending=False).reset_index(drop=True)


def apply_merge_mapping(brands, mapping):
    import numpy as np
    if not mapping:
        return brands
    norm_map = {normalize_brand(k).lower(): normalize_brand(v) for k, v in mapping.items()}
    lower = brands.fillna("").astype(str).map(lambda x: normalize_brand(x).lower())
